...
CIRCLE_R_MIN = 8
CIRCLE_R_MAX = 80
FULL_CIRCLE_THRESHOLD = 2000
FULL_CIRCLE_MIN_CONF = 2000
CIRCLE_REL_X_MIN = 15
CIRCLE_REL_X_MAX = 85
CIRCLE_REL_Y_MIN = 25
CIRCLE_REL_Y_MAX = 75
...
def choose_circle_center(circles, corners):
    ...
    for c in circles:
        rel_y = (c.y() - y1) * 100 // rh
        rel_x = (c.x() - x1) * 100 // rw
        if (CIRCLE_REL_Y_MIN <= rel_y <= CIRCLE_REL_Y_MAX and
                CIRCLE_REL_X_MIN <= rel_x <= CIRCLE_REL_X_MAX):
            filtered.append(c)
    if not filtered:
        return None
...
def choose_full_circle(circles):
    best = None
    best_score = -1
    for c in circles:
        if c.x() < 20 or c.x() > WIDTH - 20:
            continue
        if c.y() < 20 or c.y() > HEIGHT - 20:
            continue
        score = c.magnitude() + c.r() * 20
        if score > best_score:
            best = c
            best_score = score
    if best is None:
        return None
    return best.x(), best.y(), best.magnitude()
...
def detect_target(img):
        ...
        if circles:
            ...
            if center is not None:
                ...
                if conf >= CIRCLE_MIN_CONF:
                    return cx - CENTER_X, cy - CENTER_Y, conf, "CIRCLE_CENTER"

    circles = img.find_circles(
        roi=(0, 0, WIDTH, HEIGHT),
        threshold=FULL_CIRCLE_THRESHOLD,
        x_margin=10,
        y_margin=10,
        r_margin=10,
        r_min=CIRCLE_R_MIN,
    ...
    center = choose_full_circle(circles)
    if center is None:
        return None
    cx, cy, conf = center
    if conf < FULL_CIRCLE_MIN_CONF:
        return None
    return cx - CENTER_X, cy - CENTER_Y, conf, "CIRCLE_CENTER"
